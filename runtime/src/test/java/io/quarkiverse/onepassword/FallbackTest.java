...
class FallbackTest {
    ...
    @BeforeEach void captureLogs() { logger.addHandler(capture); }
    @AfterEach void stopCapture() { logger.removeHandler(capture); }
    ...
    @Test void literalDefaultWarnsWithoutExposingValues() throws Exception {
        ...
        assertFalse(logs.stream().map(r -> r.getMessage() + Arrays.toString(r.getParameters())).toList().toString().contains("SENSITIVE"));
        assertFalse(logs.stream().map(r -> r.getMessage() + Arrays.toString(r.getParameters())).toList().toString().contains("PRIVATE-DEFAULT"));
    }
    @Test void missingDefaultReportsPropertyAndLookupReason() throws Exception {
    ...
    @Test void directRequiredSecretPreservesReason() throws Exception {
        var c = config("exit 7", Map.of("app.password", "${op::op://v/i/p}"));
        var error = assertThrows(OnePasswordException.class, () -> c.getValue("app.password", String.class));
        assertTrue(error.getMessage().contains("exit 7"));
    }
    @Test void nestedDefaultsResolveThroughSmallRye() throws Exception {
    ...
    @Test void successPreservesExpressionLookingSecretData() throws Exception {
    ...
    @Test void explicitOverrideAvoidsLookupAndWarnings() throws Exception {
        var c = config("exit 7", Map.of("vault.password", "${op::op://v/i/p}",
                "app.password", "${OVERRIDE:${vault.password:local}}", "OVERRIDE", "override"));
        assertEquals("override", c.getValue("app.password", String.class));
        assertTrue(logs.isEmpty());
    }
    @Test void malformedReferenceIsNotHiddenByDefault() throws Exception {
