...
class CachingOnePasswordResolverTest {
    @Test
    void concurrentRequestsShareOneLookup() throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < 32; i++)
                futures.add(pool.submit(() -> cache.resolve("op://v/i/p")));
            for (var f : futures)
                assertEquals("value-1", f.get(3, TimeUnit.SECONDS));
    ...
    void failuresAreRetried() {
        ...
        var cache = new CachingOnePasswordResolver(ref -> {
            if (calls.incrementAndGet() == 1)
                throw new OnePasswordException("Denied");
            return "ok";
        }, 2);
    ...
    @Test
    void evictsLeastRecentlyUsedAndDoesNotShareAcrossInstances() {
        AtomicInteger calls = new AtomicInteger();
